...
\section{ 関連研究}
本章では本稿に関連する研究について説明する．
 \subsection{Janus}
\texttt{Janus}とは可逆プログラミング言語の一種で，多重集合と配列の書木換えに基づく制約処理モデルを持つ．\texttt{Janus}は，C言語に似た構文に加えて可逆性を保
証するための構文規則を持つ．
...
　　　\caption{チューリング機械の様式図}\label{fig:TM}
...
\end{array}
\]
\caption{\texttt{R-WHILE}     $\mathcal{I}$}\label{fig:i}
\end{figure*}
