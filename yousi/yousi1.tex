...
また，任意のTMを模倣できる計算システムは計算万能(チューリング完全)性をもつといわれる.
...
\texttt{WHILE}言語とは，ローカル変数，整数，ブール式をもつ単純な命令型言
語である.\texttt{WHILE}は構文規則や意味規則が他のプログラミング言語と比
べ少なく，TMを模倣できるくらいの計算能力を持っているため，プログラムやプ
...
\vspace{-8.0mm}
 ...
 一方3項組は，遷移規則の数は5項組と比べてヘッドの移動とテープの記号の書き
...
\texttt{STEP(Q,T)}を実行することによる状態の変化が対応していることを示
した．RTMの様相を表す$(q,(l,s,r))$は$(q,(l,s,r))\in Q\times ((\Sigma \backslash \{b\})^* \times \Sigma\times (\Sigma \backslash \{b\})^*)$である．ここでは，$\mathcal{T}\jump{(q,(l,s,r))}$ = \{\texttt{Q} $\mapsto${$\overline{q}$},
...
\texttt{R-WHILE}の計算モデルは可逆的にチューリング完全である．すなわち，可逆プログラ
