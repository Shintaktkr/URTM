...
任意のTMを模倣できる計算システムは計算万能性(チューリング完全)をもつといわれる.
プログラミング言語の計算モデルが計算万能性をもつことを示すことは重要である．

可逆計算とは，計算過程のどの状態においても直前と直後にとり得る
状態を高々1つもつものである．
計算機における非可逆計算ではエネルギーが消
費されることがわかっている\cite{5}．一方，
可逆計算では，理想的な状況下ではその消費を0にすることができ，情報の
消失が発生しない．
% 可逆計算システム
% が計算できるのは単射な計算可能関数であることが知られている\cite{AxGl11b}．
% 可逆チューリング機械とはチューリング機械の計算過程が常に可逆であるよう
% に設計されたものである．
可逆チューリング機械(以下，RTM)が計算できるのは単射な計算可能関数である
ことが知られている\cite{2}．

可逆プログラミング言語とは，そのプログラムの実行過程が必ず可逆になるよう
な言語設計がなされているプログラミング言語である．
可逆プログラミング言語がRTMを模倣できること，すなわちその計算モデルが可逆的計算万能性をもつことを示すことは重要である．
...
ない．したがって，本稿では可逆プログラミング言語\texttt{R-WHILE}によって万能可
チューリング機械を構成することにより，\texttt{R-WHILE}が可逆的計算万能性
をもつことの証明を目的とする．
...
研究について説明する．
 \subsection{Janus}
\texttt{Janus}は\texttt{R-WHILE}と同様に命令型の可逆プログラミング言語の一種で，多重集合と配列の書木換えに基づく制約処理モデルを持つ．\texttt{Janus}は，C言語に似た構文に加えて可逆性を保証するための構文規則を持つ．
 \subsection{セルオートマトン}
セルオートマトン(以下，CA)とは1950年代に$John$ $von$ $Neumann$が自己増殖オートマトンを設計するための理論的枠組みとして提案されたモデルである．
いくつかの状態を持つセルという単位によって構成され，事前に設定された規則に従い，そのセル自身や近傍の状態によって，時間発展と共に，その時間における各セルの状態が決定される．
TMと同様に計算モデルとして，計算可能性理論を議論する際に重要である．
\subsection{可逆セルオートマトン}
可逆セルオートマトン(以下，RCA)とは，可逆性が保証されたCA，つまり，現
在の状態から1つ前の状態を一意に決めることができるCAのことである．
<<<<<<< HEAD
$Toffoli$はRCAの次元を1増やすことで、2次元以上のRCAにおいて任意のTMが模倣できることを示し、そこから分割CAの枠組みを利用して、1次元以上のRCAが計算可逆万能性を持つことを証明した。


=======
$Toffoli$はRCAの次元を1増やすことで、2次元以上のRCAにおいて任意のTMが模倣できることを示し、そこから分割セルオートマトンの枠組みを利用して、1次元以上のRCAが計算可逆万能性を持つことを証明した。
>>>>>>> b09b480a49e11b1ed85381e1d8a95ed0a3cf3ccb
\section{可逆チューリング機械}
...
\vspace{-8.0mm}
 ...
 が，5項組で設計を行った場合，ヘッドの移動とテープの記号の書き換えを1つ
 の遷移規則で行うことができる為，少ない遷移規則で記述することが出来る．
 一方3項組は，遷移規則の数は5項組と比べてヘッドの移動とテープの記号書き
 換えが別々なため多くなってしまう．しかし，遷移規則に対称性があるため，以降の
 章で説明するRTMを設計する際に前方決定的または後方決
 定的であるということの判断がしやすいため今回は3項組を用いる．\par
 ここで，TMが計算を行っているある時点での様子を様相とする．
...
\texttt{Y}を書き出す．すべての変数の初期値は\texttt{nil}である．可逆的繰
り返しを行う命令，\texttt{loop...until}は\texttt{Y}が\texttt{nil}であると主張
...
的降下によって得ることができる．以下のプログラムはプログラム
\texttt{reverse}を逆変換器$\mathcal(I)$によって得た逆プログラム$\mathcal{I}$\jump{\texttt{reverse}}である．
\vspace{-1mm}
\begin{alltt}
...
\end{alltt}
\vspace{-1mm}
　このプログラムは元のプログラム\texttt{reverse}と同じ働きをする．またプ
ログラムは\texttt{X}と\texttt{Y}の位置が入れ替わっていることを除き，同じ
である．
...
\texttt{T}はTMのテープの状態を表している．そのため，可逆的
...
\vspace{-3mm}
\end{figure}
そして，組\texttt{(Q,T)}はTMの様相を表している．プロ
...
\section{おわりに}
本稿の5章において任意のRTMから\texttt{R-WHILE}プログラムへの変換
...
した．すなわち，今回は可逆プログラミング言語\texttt{R-WHILE}によって万能
可逆チューリング機械が作れることを示すことができた．
