...
\end{center}
\vspace{-2mm}
である（このとき\texttt{L}，\texttt{R}，\texttt{Q}そして\texttt{T}は互い
 に異なる変数を表す）．この書き換え規則により，
 $\overline{q}$は$\overline{q'}$へ束縛する．また$\overline{s}$を
 $\overline{s'}$へ束縛する． 状態
...
\end{center}
\vspace{-2mm}
である．まずマクロ\texttt{MOVEL(T)}が実行され，\texttt{T}に束縛される値は
\vspace{-2mm}
\begin{center}
...
\end{center}
\vspace{-2mm}
である．まずマクロ\texttt{MOVER(T)}が実行され，\texttt{T}に束縛される値は
\vspace{-2mm}
\begin{center}
\texttt{(L} \ $\overline{s}$ \ $\overline{s'}$\texttt{.R)} \texttt{=>}
 \texttt{(L.}$\overline{s} \ \overline{s'}$ \ \texttt{R)}
...
が成り立つ．
\end{lemma}
\textbf{証明} 可逆チューリング機械の計算ステップが$n$回実行される場合，
