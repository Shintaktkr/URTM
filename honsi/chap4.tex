...
\section{R-WHILEについて}
ここでは\texttt{R-WHILE}[文献]について説明する．\\
...
　リストと似た表記として，私たちは\texttt{($\mathtt{Q_1}$.($\mathtt{Q_2}$.(・・・.($\mathtt{Q_n}$.nil)・・・)))}を
...
れる．私たちは値の範囲として$\mathbb{D}_{\bot}$を用いる．\\
...
それらは共通の変数をもたない．未定義の状態すなわち，どのような変数の対し
ても$\bot$をかえすものは$\bot$とも書かれる．\\
　状態$\mathtt{\sigma^P_X}$は変数\texttt{X}を$D$に束縛する．また，
$Var$(\texttt{P})=\{\texttt{X,$\mathtt{Y_1}$,...,$\mathtt{Y_n}$}\}内の
...
\texttt{d}の対応する部分に縛られおり，与えられるそれらの変数は状態
...
　 ここからは命令評価$\mathcal{C}$について述べる．非可逆な言語における代
 ...
 ング言語では言語では基本
 的に用いることができない．そのかわりに，可逆代入\texttt{X \textasciicircum= E}を用い
 る．これは, もし\texttt{X}の値が\texttt{nil}であるとき，\texttt{X}に
 \texttt{E}の値にし，\texttt{X}の値が\texttt{E}の値と等しいとき，
 \texttt{X}の値を\texttt{nil}にするというものである．前者の場合
...
な値であっても\texttt{X}に\texttt{nil}をセットする．そして，
...
命令の逆はそれぞれの逆命令の順序を逆にされたものである．テストとア
...
プログラムは糖類構文としての可逆版の書き換え規則をもちいることで読みやす
くなる．糖類構文によって，複数の変数
...
れるか，命令\texttt{C}が実行され，$\mathtt{X_i}$も更新される．変数が同
...
\ref{fig:reg_R}）によって生成された書き換え規則の列である．変換器
$\mathcal{T}$によって\\
...
味，プログラミング言語\texttt{R-WHILE}または\texttt{R-WHILE}の書き換えの規則の定義はすでに本稿のなかで定義されたもの
を用いる．\\
\texttt{R-WHILE}の$\mathbb{D}$を可逆チューリング機械の$\Sigma^*$へと変換す
る変換器を\texttt{R2T}とし，以下の様に定義する．\\
\texttt{R2T} : $\mathbb{D}$ → $\Sigma^*$ \\
\texttt{R2T}\jump{\texttt{nil}} = 0\\
\texttt{R2T}\jump{(\mathtt{d_1,d_2})} = 1
\texttt{R2T}\jump{\mathtt{d_1}}・\texttt{R2T}\jump{\mathtt{d_2}}\\
また，\texttt{R2T}の逆である$\Sigma^*$を$\mathbb{D}$へ変換する変換機を
\texttt{T2R}とする．\\
\textbf{定理 1}
任意の可逆チューリング機械T，任意の$x$に対して
\vspace{-2mm}
\begin{center}
   \jump{T}$x$ =
 \texttt{R2T}\jump{\jump{\mathcal{T}\jump{T}}(T2R\jump{x})}
\end{center}
\vspace{-2mm}
である．
\textbf{定理 1}を証明するにあたって\textbf{補題 1}と\textbf{補題
 2}を与える．ここでは変換器$\mathcal{T}$によって変換されたものを
 $\overline{・}$をつかって記述する．\\
\textbf{補題 １}任意の様相$\sigma$ = $(q,(l.s.r))$，$\sigma'$ =
$(q'(l',s',r'))$に対して，
\vspace{-2mm}
\begin{center}
...
$\overline{\sigma}$ = $(\overline{q,(l,s,r)})$ = \{\texttt{Q} $\mapsto$
 ({$\overline{q}$}), \texttt{T} $\mapsto$ ($\overline{l}$,$\overline{s}$,$\overline{r}$)\}
\end{center}
\vspace{-2mm}
として，
\vspace{-2mm}
...
\end{center}
\vspace{-2mm}
が成り立つ．このとき，$\mathcal{C}$\jump{}は対象がプログラムであることを示して
いる．\\
$[$証明$]$\textbf{補題 1}を証明する為に，可逆チューリング機械における４パター
ンの遷移規則において，\textbf{補題 1}が成り立つことを示す．
\texttt{R-WHILE}で表記するにあたって，$\overline{l}$ = L，$\overline{s}$ =
Sまたは$\overline{r}$ = Rとする．\\
\\
...
\vspace{-2mm}
\begin{center}
$\mathcal{T}$ $\jump{[q,\langle s,s'\rangle ,q']}$ =
 $[\overline{q},$(\texttt{L,S,R})$]$ \texttt{=>} $[\overline{q'},$(\texttt{L,S',R})$]$
\end{center}
\vspace{-2mm}
によって
\vspace{-2mm}
\begin{center}
$[\overline{q'},$(\texttt{L,S',R})$]$
\end{center}
\vspace{-2mm}
となり，
\vspace{-2mm}
\begin{center}
 $[\overline{q'},$(\texttt{L,S,R})$]$ = $(q',(l,s',r))$
\end{center}
\vspace{-2mm}
である．\\
\\
...
\vspace{-2mm}
\begin{center}
$[\overline{q'},$(\texttt{L,S',SR})$]$
\end{center}
\vspace{-2mm}
となり，
\vspace{-2mm}
\begin{center}
 $[\overline{q'},$(\texttt{L,S',SR})$]$ = $(q',(l,s',sr))$
\end{center}
\vspace{-2mm}
である．\\
\\
...
\vspace{-2mm}
\begin{center}
$[\overline{q'},$(\texttt{LS,S',R})$]$
\end{center}
\vspace{-2mm}
となり，
\vspace{-2mm}
\begin{center}
 $[\overline{q'},$(\texttt{LS,S',R})$]$ = $(q',(ls,s',r))$
\end{center}
\vspace{-2mm}
である．\\
\\
...
ある．\\
\textbf{補題 2} 任意の様相$\sigma$ = $(q,(l.s.r))$，$\sigma'$
=$(q'(l',s',r'))$または自然数$n$に対して，
\vspace{-2mm}
\begin{center}
...
\vspace{-2mm}
\begin{center}
($\mathcal{C}\jump{\texttt{STEP(Q.T)}})^n$
\end{center}
\vspace{-2mm}
が成り立つ．\\
$[$証明$]$可逆チューリング機械の計算ステップが$n$回実行される場合，
...
\vspace{-2mm}
\begin{center}
$\sigma$ $\xRightarrow[T]{}^k$ $\sigma'$ $\Longleftrightarrow$ ($\mathcal{C}\jump{\texttt{STEP(Q.T)}})^k$
\end{center}
\vspace{-2mm}
である．$n = k + 1$のとき
\vspace{-2mm}
\begin{center}
($\mathcal{C}\jump{\texttt{STEP(Q.T)}})^{k+1}$
\end{center}
\vspace{-2mm}
であり，
\vspace{-2mm}
\begin{center}
($\mathcal{C}\jump{\texttt{STEP(Q.T)}})^{k+1}$　=
 ($\mathcal{C}\jump{\texttt{STEP(Q.T)}})^1$($\mathcal{C}\jump{\texttt{STEP(Q.T)}})^{k}$
\end{center}
\vspace{-2mm}
であるため，$n = k + 1$のときも\textbf{補題 2}は成立する．\\
　(i)，(ii)よりすべての自然数$n$について\textbf{補題 2}は成立する．\\
\textbf{補題 1}，\textbf{補題 2}より\textbf{定理 1}は成り立つ．
\begin{figure}[t]
