...
　マクロ\texttt{STEP(Q.T)}は図\ref{fig:rtmprog:STEP}で定義し
 たように，可逆チューリング機械の遷移規則に応じて\texttt{[Q.T]}が書き換
 える．なお，可逆チューリング機械は局所的に前方決定的かつ局所的に後
 方決定的であるという性質をもつため，どの状態においても遷移規則はただ一
...
\end{center}
\vspace{-2mm}
である（このとき\texttt{L}と\texttt{R}は\texttt{Q}と\texttt{T}とは異
 なる全称記号であり，任意の記号列を表している）．この書き換え規則により，
